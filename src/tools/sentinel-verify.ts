...
function detailsFor(extra: Partial<VerifyDetails> = {}): VerifyDetails {
  ...
  export function createSentinelVerifyTool(runtime: SentinelRuntime) {
    return {
    ...
    async execute(
      ...
      // they drive both evidence and the code-graph impact section. This tool is
      // an explicit "check now", so nothing is widened by graph dependents: the
      // run reports on the turn, not on its blast radius.
      const focusPaths = runtime.snapshots.turnPaths();
      ...
      if (doRollback && !failure.warnOnly) {
        const rb = rollbackTurn(cwd, runtime.snapshots);
