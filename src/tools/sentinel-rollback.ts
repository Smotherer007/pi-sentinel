 ...
 *
 * Built from the session runtime, because "the current turn" only means
 * something relative to the snapshots *this* session captured.
 */
...
export function createSentinelRollbackTool(runtime: SentinelRuntime) {
  return {
    ...
    async execute(
      ...
      if (mode === "turn" && !runtime.snapshots.hasTurnSnapshot()) {
        return {
          content: [
            {
              type: "text" as const,
