 ...
 *
 * Built from the session runtime rather than from a module-level store, so the
 * tool acts on the checkpoints of *this* session.
 */
...
export function createSentinelRewindTool(runtime: SentinelRuntime) {
  return {
    ...
    async execute(
      ...
      const list = runtime.checkpoints.list(cwd, 10);
      ...
      const report = runtime.checkpoints.restore(cwd, targetId);
      if (!report.attempted) {
        return {
          content: [
            {
