...
export function createSentinelStatusTool(runtime: SentinelRuntime) {
  return {
    ...
    async execute(
      ...
      const checkpointList = runtime.checkpoints.list(cwd, 5).map((cp) => {
      ...
      const trackedEscalations = runtime.escalations.snapshot();
      ...
      return {
        ...
        details: {
          ...
          git: repo,
          turnSnapshot: runtime.snapshots.hasTurnSnapshot(),
          checkpoints: runtime.checkpoints.list(cwd, 5),
